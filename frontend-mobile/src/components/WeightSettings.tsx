...
export default function WeightSettings() {
  ...
  return (
    <View className="mb-6 rounded-xl border border-border bg-surface p-5">
      <Text className="mb-4 text-lg font-semibold text-fg">{t("trackingSettings.weight.title")}</Text>
      {error ? <Text className="mb-4 text-sm text-danger">{error}</Text> : null}

      <View className="mb-4 flex-row items-center justify-between gap-3">
        <Text className="shrink text-fg">{t("trackingSettings.weight.account")}</Text>
        <Switch
      ...
      {!user.weight_tracking_enabled ? (
        <Text className="text-sm text-muted">
          {t("trackingSettings.weight.off")}
        </Text>
      ) : null}

      {user.weight_tracking_enabled && pets.length === 0 ? (
        <Text className="text-sm text-muted">{t("trackingSettings.weight.noPets")}</Text>
      ) : null}

...
// Logs in the user by sending their email and password to the API. If successful, it stores the returned token in localStorage.
...
// Fetches the current user's information from the API. If the token is invalid or expired, it will clear the token from localStorage.
...
export async function updateMe(data: { reminders_enabled?: boolean; reminder_frequency?: User["reminder_frequency"]; push_enabled?: boolean; weight_tracking_enabled?: boolean; timezone?: string; username?: string }): Promise<User> {
  return apiFetch<User>("/auth/me", {
    method: "PATCH",
